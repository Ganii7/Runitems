        ...
        for i, items in enumerate(loaded_aatrox[0]["itemBuilds"][0]["blocks"]):
            for x, item in enumerate(items["items"]):
                if i == 1 or (i >= 1 and x == 0 and (len(loaded_build["blocks"][1]["items"]) < 6)):
                    ...
                    final_objects, loaded_build = add_item(item, final_objects, loaded_build, 0)
                else:
                    alternative_items.append(item)

        for alternative_item in alternative_items:
            final_objects, loaded_build = add_item(alternative_item, final_objects, loaded_build, 2)
